...
func TestCgroupMissedTrackingErrMetrics(t *testing.T) {
	...
	// Look for an ENOENT (2) error from bpf_cgroup_rmdir.c or bpf_cgroup_release.c
	foundMissedTrackingError := false
	for _, entry := range entries {
		if entry.Error == 2 && // ENOENT
			(entry.FileName == "bpf_cgroup_rmdir.c" || entry.FileName == "bpf_cgroup_release.c") {
	...
	assert.True(t, foundMissedTrackingError,
		"Expected to find ENOENT error metric from bpf_cgroup_rmdir.c or bpf_cgroup_release.c for missed cgroup tracking")
}
